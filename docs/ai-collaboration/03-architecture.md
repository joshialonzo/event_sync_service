...
One container: the FastAPI app under Uvicorn with hot-reload, serving both the JSON API and the HTML
pages. It runs the ingest and reconciliation pipeline on startup, so the dataset is populated the
moment the service is reachable — there is no seeding step to forget. No account, no credentials, no
network needed to evaluate this project.
...
│   │   ├── templates/             # Jinja2: base, meetings, detail, stats
