...
`Settings` (pydantic-settings): `data_dir` (default `../data`, env-overridable), `timezone`
(`America/New_York`). Health response includes the resolved data dir.
...
**Test:** `python -c "from app.models.normalized import NormalizedEvent; print(NormalizedEvent.model_json_schema()['required'])"`.

### Step 5 — Unified models
**Files:** `backend/app/models/unified.py`
`ProvenanceField`, `ConflictKind`, `MatchEvidence`, `UnifiedMeeting`, `SyncRunSummary`, `SyncResult`.
**Test:** construct one `UnifiedMeeting` in a REPL and `model_dump_json()` it — the shape must match
the JSON block in doc 02, Decision 4.

---

## Phase 3 — Normalize (doc 02, Decision 1)

### Step 6 — Parsing primitives
**Files:** `backend/app/reconcile/__init__.py`, `backend/app/reconcile/parse.py`, `backend/tests/test_parse.py`
...
### Step 7 — CRM normalizer
**Files:** `backend/app/reconcile/normalize_crm.py`, `backend/tests/test_normalize_crm.py`
...
### Step 8 — Calendar normalizer
**Files:** `backend/app/reconcile/normalize_calendar.py`, `backend/tests/test_normalize_calendar.py`
...
### Step 9 — Intra-source dedupe (doc 02, Decision 2)
**Files:** `backend/app/reconcile/dedupe.py`, `backend/tests/test_dedupe.py`
...
### Step 10 — Match signals (doc 02, Decision 3)
**Files:** `backend/app/reconcile/signals.py`, `backend/tests/test_signals.py`
...
### Step 11 — Matcher
**Files:** `backend/app/reconcile/matcher.py`, `backend/tests/test_matcher.py`
...
### Step 12 — Merge (doc 02, Decision 4)
**Files:** `backend/app/reconcile/merge.py`, `backend/tests/test_merge.py`
...
**Test:** REPL — `replace_all` twice, confirm `list_meetings` reflects only the latest.

### Step 14 — Sync job
**Files:** `backend/app/jobs/__init__.py`, `backend/app/jobs/sync.py`, `backend/tests/test_sync.py`
...
set from Step 12, a date range narrows the list.

### Step 18 — Stats and re-sync
**Files:** `backend/app/api/routes.py`
...
### Step 21 — Filter controls
**Files:** `backend/app/web.py`, `backend/app/templates/meetings.html`
A plain `GET` form whose inputs are the Step 17 query params; selections persist in the rendered
form.
...
### Step 25 — Container and single command
...
### Step 26 — Reconcile the docs with the built repo
...
1. `pytest` stays green; the 17-pair fixture never regresses.
